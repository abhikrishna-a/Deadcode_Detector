        ...
        for fr in results_list:
            ...
            if submission:
                if is_error:
                    ...
                    submission.error = ""
                    submission.save(
                        update_fields=["analysis_id", "rag_document_id", "result", "status", "completed_at", "error"]
                    )
                    _notify_user(
                        submission.user_id,
                        {
    ...
    try:
        ...
        for senior in user_model.objects.filter(role="senior").iterator():
            async_to_sync(channel_layer.group_send)(
                f"notifications_user_{senior.id}",
                message,
            )
    except Exception:
...
def analyze_junior_submission(self, submission_id: int):
    ...
    try:
        ...
        if not resp.ok:
            logger.error(
                "RAG returned %d for submission %d (%s): %s",
                resp.status_code,
                submission_id,
                submission.filename,
                resp.text[:500],
        ...
        if exc.response is not None and exc.response.status_code == 400 and is_unsupported:
            ...
            logger.warning(
                "Permanently failing submission %d (%s) — unsupported file type", submission_id, submission.filename
            )
            _notify_user(
                user.id,
                {
