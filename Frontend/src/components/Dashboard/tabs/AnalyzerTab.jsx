...
export default function AnalyzerTab({ results, onResultsChange, onFileChange, file }) {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      onResultsChange({ filename: result.filename, ...result.analysis });
    } catch (err) {
      onResultsChange({
  ...
  return (
    ...
    >
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, alignItems: 'start' }} className="analyzer-grid">
        {/* LEFT PANEL */}
        <div>
          <div
            style={dropZoneStyle}
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {[
                ...
                { label: 'Unused classes', desc: 'Defined but never instantiated' },
                { label: 'Dead imports', desc: 'Imported but not used' },
                { label: 'Unreachable code', desc: 'After return, raise, break' },
                { label: 'Unused variables', desc: 'Assigned but never read' },
                { label: 'Dead branches', desc: 'Conditions that are always false' },
                { label: 'Commented code', desc: 'Large blocks left in' },
              ].map(r => (
          ...
          {ragUploading && (
            <div style={{
              ...
              <p style={{ fontSize: 12, color: '#fb923c', fontFamily: "'DM Mono', monospace", marginBottom: 8 }}>Analyzing with Grok…</p>
              <div style={{
